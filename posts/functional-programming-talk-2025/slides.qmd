...
A category is the composition of all objects and arrows (in the graph).
...
- Used to hold other types.

- There are two kinds: Sum types and Product types.

- Really important when composing types.

## Algebraic data types: Sum types {.center}

-   Combination is the sum of the types inside.

-   Example: Number of values for a set of Colours (Red, Green, Blue) is 1 + 1 + 1 = 3
...
-   Combination is the product of the types inside.

-   Example: Number of values for set of (Colours, Bool) is 3 \* 2 = 6.
...
Read as "f composed into g" or "f followed by g"

## Composing via piping {.center}

Mathematically, you can "chain" or "pipe" functions together, so these
are the same:
...
A type that lets you apply an action to types inside of the type without
changing the structure. E.g. lists are usually functors.
...
## Higher order functions (functors) that take a function as an input {.center}
...
## A data frame is a functor {.center}

-   You can apply actions to the columns without changing the data frame
    type.
...
graph TB
  ...
  B -->|"remove missing"| C(Data: No missingness)
  C -->|"standardize column names"| D(Data: Cleaned)
```
...
## Mapping a function over a list of items
...
-   Object types depend on the package you use.
...
`Direction` can only have one value and there are four
possible values.

::: notes
In Rust, you create the object, which defines the type. A type doesn't need
a value. Rust more closely follows functional
programming, so this is the object construction.
:::
...
## Illustration of a monad {.center}

```{mermaid}
graph LR
    In(Sum type:<br>Type A<br>Type B) -->|M f| Out(Type A)
```

## Example: Option type in Rust
...
This is really nice because it forces you to handle None cases explicitly,
since not all functions take `Option` as an input type.
:::
...
// with either `?` or `expect()` or `match`
