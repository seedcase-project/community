...
matches our ethos and needs. This second learning short is a continuation of [A
brief introduction to functional
programming](functional-programming-theory.qmd), in which we discussed the
basics of functional programming, its advantages, and how it differs
...
programming in Python without going into too much theoretical or conceptual detail.
...
functions `filter()`, `map()`, and the `reduce()` function from `functools` to
process collections of data and understand how to use the `lambda`
...
many functional programming features are built-in. This
style encourages writing clean, concise, and expressive code---especially
when working with data transformations.
...
used in conjunction with `filter()`, `map()`, and `reduce()` to create more
...
the result, we need to convert it to a list with `list()`. We do the same below with
the `map()` function.
...
-   **List comprehensions & generator expressions**: Provide a
    syntax for transforming data iterables that might be more familiar to those
    used to loops and object-oriented programming. It is not a functional programming
    tool, though can help transition to those concepts.
