...
creating. Complexity, which is anything related to the structure of the
code or text that makes it hard to understand and modify, is managed by
effective design. But complexity isn't a single thing, it is a
...
2.  Encapsulate the complexity in a way that exposure to the complexity
    is less and may not happen all at once. This is called [modular
...
includes it's "signature" (e.g. the function name and parameters) as well as the type of errors it outputs.
...
A module can be different "depths", with a spectrum between "deep" and "shallow":
