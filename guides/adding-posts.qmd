...
Contributing Guide Entries](/guides.qmd) is
a collection of post entries).

There are two ways to add a post:

1.  Cloning the
    [`community`](https://github.com/seedcase-project/community)
...
1.  Create a new file inside `posts/` with a short
    name the describes the post. For instance, this post's file
    name is `adding-posts.qmd`.
...
5.  After you are done writing, the final step is to get the post into
    the `community` Git history. This step depends on if you
