...
learning short is very short, only taking a few minutes to read. We aim to keep shorts concise, with a reading time under 10 minutes.
...
The `shorts/` folder of the learning section contains a template
for writing these shorts called `_template.qmd`. The template is a
Markdown file that contains the following structure:
...
-   **Learning goal**: This is the most important part of the short,
    aimed to inform the reader of what knowledge or skill will they gain.
    It should be as simple and short as needed to convey the learning
    goal. It should be written in a form based on [Bloom's
...
-   **Lesson content**: This is the main part of the short, where the
    ...
    sections (with several headers denoted by `###`). Try to make use of images, diagrams, tables, and
    other visual aids to help explain the content. If it is code based,
    make sure to actually show code.
